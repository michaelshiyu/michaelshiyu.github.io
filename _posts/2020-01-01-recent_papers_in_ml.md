...
Title format in this post: **paper title (version #)**.

**version #**: Some of the papers were pretty new when I read them and the authors might have published newer versions afterwards.
...
1. [Papers and Comments](#papers-and-comments)

## List of Papers
A running list of papers reviewed on this page:

- [FixMatch: Simplifying Semi-Supervised Learning with Consistency and Confidence (version: arXiv v1)](#fixmatch-simplifying-semi-supervised-learning-with-consistency-and-confidence-version-arxiv-v1) 
- [A Simple Framework for Contrastive Learning of Visual Representations (version: arXiv v1)](#a-simple-framework-for-contrastive-learning-of-visual-representations-version-arxiv-v1)
- [AttnGAN: Fine-Grained Text to Image Generation with Attentional Generative Adversarial Networks (version: arXiv v1)](#attngan-fine-grained-text-to-image-generation-with-attentional-generative-adversarial-networks-version-arxiv-v1)
- [Subclass Distillation (version: arXiv v1)](#subclass-distillation-version-arxiv-v1)
- [Neural Kernels Without Tangents (version: arXiv v2)](#neural-kernels-without-tangents-version-arxiv-v2)
- [Deep Neural Networks as Gaussian Processes (version: ICLR 2018)](#deep-neural-networks-as-gaussian-processes-version-iclr-2018)
- [Neural Tangent Kernel: Convergence and Generalization in Neural Networks (version: NeurIPS 2018)](#neural-tangent-kernel-convergence-and-generalization-in-neural-networks-version-neurips-2018)
- [On Exact Computation with an Infinitely Wide Neural Net (version: NeurIPS 2019)](#on-exact-computation-with-an-infinitely-wide-neural-net-version-neurips-2019)
- [A Theoretical Analysis of Contrastive Unsupervised Representation Learning (version: ICML 2019)](#a-theoretical-analysis-of-contrastive-unsupervised-representation-learning-version-icml-2019)
- [Putting an End to End-to-End: Gradient-Isolated Learning of Representations (version: NeurIPS 2019)](#putting-an-end-to-end-to-end-gradient-isolated-learning-of-representations-version-neurips-2019)
- [Taskonomy: Disentangling Task Transfer Learning (version: CVPR 2018)](#taskonomy-disentangling-task-transfer-learning-version-cvpr-2018)
- [Similarity of Neural Network Representations Revisited (version: ICML 2019)](#similarity-of-neural-network-representations-revisited-version-icml-2019)
- [Task2Vec: Task Embedding for Meta-Learning (version: ICCV 2019)](#task2vec-task-embedding-for-meta-learning-version-iccv-2019)
- [Transferability and Hardness of Supervised Classification Tasks (version: ICCV 2019)](#transferability-and-hardness-of-supervised-classification-tasks-version-iccv-2019)
- [LEEP: A New Measure to Evaluate Transferability of Learned Representations (version: arXiv v1)](#leep-a-new-measure-to-evaluate-transferability-of-learned-representations-version-arxiv-v1)
...
**keywords:** weakly-supervised classification 

**summary:**
Considers the case where only a complementary label (a class that the example does not belong to) is provided for each example. 
Proposed a risk minimization framework to solve this problem directly.
Showed that the proposed complementary loss to be minimized unbiasedly estimates the actual classification risk under conditions, and provided error bound on the estimation.

<details>
<summary>more details</summary>
- Sec. 2 described an interesting abstraction for typical muti-class classification that is slightly different from the common ones: This abstraction highlights how typical classifiers are actually an argmax operation over a set of one-class-versus-the-rest classifiers.
- Methodology
  - The goal is to minimize the true classification risk (Eq. 1) using only complementarily labeled training examples (beginning of Sec. 3).
  - Th. 1 states that under assumptions (Eq. 5 & 7), the true classification risk can be reformulated in terms of a proposed complementary loss (involving only complementarily labeled training data) and constants. 
  This gives rise to an empirical estimation of the true risk using a sample average of complementary loss on complementarily labeled training data (Eq. 8).
  - Th. 2 states that under a easy-to-check condition (Eq. 11), loss functions in the forms of Eq. 9 and 10 satisfy the constraint of Th. 1.
  Immediately following Th. 2, some example loss functions are provided.
  - Lem. 3 & 4 bounds the Rademacher complexities in terms of complementarily labeled data using Rademacher complexities in terms of unlabeled data.
  - Lem. 5 bounds the estimation error between the empirical complementary loss (Eq. 8) and the true classification risk (Eq. 1).
  - Th. 6 bounds the difference in true classification risk between a solution obtained by minimizing an empirical complementary loss and one obtained by minimizing the true risk.
  This depends mainly on training sample size and the Rademacher complexity of the underlying model.
  And the difference converges to 0 as training sample size tending to infinity.
  For simple models such as a kernel machine, the convergence is at a rate of 1/\sqrt{n} in probability.
  - Sec. 5 describes a strategy to incorporate ordinarily labeled data into the proposed framework of learning from complementary labels.
  Specifically, the authors proposed to minimize a combination of a standard classification loss (in terms of ordinary labels) and a complementary loss.

- Results

**code:** [available](https://github.com/takashiishida/comp) 
...
**summary:** Combines two existing semi-supervised learning techniques, consistency regularization and pseudo-labeling, in a simplistic way but achieves state-of-the-art results (in particular, 94.93%/88.61% on CIFAR-10 with 250/40 labels). 
...
Is essentially a loss function of two terms: cross-entropy on weakly augmented labeled examples (Eq. 3) and cross-entropy on strongly augmented, unlabeled examples using (only confident enough) model predictions (on weakly augmented versions of these images) as artificial labels (Eq. 4). 
...
**summary:** Equated common operations used to form layers of neural networks to kernels, and studied the correlation between performance of CNNs (consisting of convolution, average pooling, and ReLU layers) and that of the corresponding (compositional) kernels. 
...
**summary:** Extended the classic result that equated a one-hidden-layer MLP to a Gaussian process (GP) when the hidden layer width goes to infinity to arbitrary MLPs. 
...
**summary:** Related the training dynamics and generalization of MLPs with the so-called neural tangent kernels (NTKs). 
